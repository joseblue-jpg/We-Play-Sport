...
export const EventInfo = () => {
  ...
  const { user } = useContext(KingOfTheCourtContext);
  ...
  useEffect(() => {
    ...
    const fetchEvents = async () => {
      try {
        let result = await fetchData(`/events/oneEvent/${id}`, 'get');
        setEventUnico(result.data);
      } catch (error) {
        console.log(error);
      }
    };
    ...
    const fetchEventIsFull = async ()=>{
      try {
        await fetchData(`/events/eventIsFull/${id}`, `post`);        
    ...
    const fetchServices = async () => {
      try {
        let result = await fetchData(`/events/getServicesFromAnEvent/${id}`, 'get');
  ...
  const handleInscription = () => {
    if (!user) {
      ...
      Swal.fire({
        title: "¡Atención!",
        text: "Debes registrarte o iniciar sesión para inscribirte en este evento.",
        icon: "warning",
        ...
        if (result.isConfirmed) {
          ...
          navigate("/login"); // Redirige a la página de login si el usuario opta por "Iniciar sesión"
        }
      });
    } else {
      // Si el usuario está registrado, lo redirige a la inscripción
      navigate(`/inscriptionEvent/${eventUnico.event_id}`);
    }
  };
        ...
        <div className='event-info-main container-xl p-3'>
          ...
          <div className='d-flex flex-column justify-content-center my-3'>
            ...
            <div className='d-flex my-2 gap-2'>
              {eventUnico?.status === 2 && (
                <button
                  className='border-0 rounded-3 px-5 py-2 fw-bold'
          ...
          <div className='info-main-div d-flex gap-3'>
            <div className='info-div rounded-2'>
              ...
              <div className='px-4 py-2'>
                ...
                <p><strong>Duración del evento:</strong> Desde el {eventUnico.date_start} hasta el {eventUnico.date_end}</p>
          ...
          <div className='d-flex justify-content-center'>
            <button className=' back-btn py-2 px-4 rounded-3 border-0 text-white mt-3' onClick={() => navigate(`/`)}>VOLVER</button>
          </div>
        </div>
      </div>
    </>
